    ...
    def limit_data_range(self, ra_range=None, dec_range=None):
        if ra_range is not None:
            if type(ra_range)==int:
                ...
                ramin = np.argmin(abs(np.subtract(self.ra_axis,ramid - ra_range/2)))
                ramax = np.argmin(abs(np.subtract(self.ra_axis,ramid + ra_range/2)))
                use_ra_inds = [i for i in range(ramin,ramax)]
                ra_range = [self.ra_axis[ramin],self.ra_axis[ramax]]
            else:
